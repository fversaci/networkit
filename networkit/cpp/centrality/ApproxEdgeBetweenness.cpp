// no-networkit-format
/*
 * ApproxEdgeBetweenness.cpp
 *
 *  Created on: 18.05.2022
 *      Author: cesco
 */

#include <networkit/centrality/ApproxEdgeBetweenness.hpp>
...
namespace NetworKit {
  ...
  ApproxEdgeBetweenness::ApproxEdgeBetweenness(const Graph& G, double epsilon, double delta, double universalConstant) : Centrality(G, true, true), epsilon(epsilon), delta(delta), universalConstant(universalConstant) {

}


void ApproxEdgeBetweenness::run() {
    bool computeEdgeCentrality = true; // computeEdgeCentrality by default
	...
	std::cout << edgeScoreData.size() << std::endl;
    }
...
#pragma omp for
        for (omp_index i = 1; i <= static_cast<omp_index>(r); i++) {
		    ...
		    // Edge centrality
		    double upd_val = 1. / static_cast<double>(r);
		    if (computeEdgeCentrality) {
		      const edgeid edgeId = G.edgeId(z, t);
#pragma omp atomic
		      edgeScoreData[edgeId] += upd_val;
		    }		    
		    /////////////////
                    if (z != u)
#pragma omp atomic
...
count ApproxEdgeBetweenness::numberOfSamples() const {
    assureFinished();
